...
function remap(input: number, inLow: number, inHigh: number, outLow: number, outHigh: number) {
  return ((input - inLow) * (outHigh - outLow)) / (inHigh - inLow) + outLow;
}

function remapClamp(input: number, inLow: number, inHigh: number, outLow: number, outHigh: number) {
  return clamp(
...
function signedModDelta(a: number, b: number, mod: number) {
  let delta = (b - a + mod) % mod;
  if (delta > mod / 2) delta -= mod;
  return delta;
}

function segmentCircleIntersection(x1: number, y1: number, x2: number, y2: number, cx: number, cy: number, r: number) {
  const dx = x2 - x1;
  const dy = y2 - y1;
...
const TAU = 2 * Math.PI;
var mod = function (a: number, n: number) {
  return ((a % n) + n) % n;
}; // modulo

var equivalent = function (a: number) {
  return mod(a + Math.PI, TAU) - Math.PI;
}; // [-π, +π]

function smallestAngle(current: number, target: number) {
...
function lineIntersectsCircle(p1: { x: number; y: number }, p2: { x: number; y: number }, center: { x: number; y: number }, radius: number) {
...
function pointInPolygon(point: { x: number; y: number }, poly: { x: number; y: number }[]) {
  let inside = false;
...
function circleIntersectsPolygon(center: { x: number; y: number }, radius: number, poly: { x: number; y: number }[]) {
